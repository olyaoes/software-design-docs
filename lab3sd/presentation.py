...
class OnboardingController:
    ...
    def _setup_routes(self):
        ...
        @self.router.post("/quick-add-factory")
        async def quick_add_factory():
            self.service.add_via_factory()
            return RedirectResponse(url="/", status_code=303)

        @self.router.post("/quick-add-adapter")
        async def quick_add_adapter():
            self.service.add_via_adapter()
            return RedirectResponse(url="/", status_code=303)
        ...
        @self.router.post("/run-lab4")
        async def execute_lab4(request: Request):
            form_data = await request.form()
            selected_strategy = form_data.get("output_strategy")
            lab4_main.run_lab4_process(selected_strategy)
            return RedirectResponse(url="/", status_code=303)